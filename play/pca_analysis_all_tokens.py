...
def process_activations(activations: torch.Tensor) -> np.ndarray:
    ...
    # Remove only BOS token (position 0), keep all other positions including final token
    num_block = activations.shape[1]
    activations_reduced = activations[:, 1:, :]  # Shape: (num_sequences, seq_len-1, n_embd)
...
def compute_theoretical_belief_states(sequences: List[List[int]], constrained: bool = False) -> np.ndarray:
    ...
    Returns:
        Array of shape (num_sequences * (seq_len-1), 3) containing belief states
        Excludes only BOS token (position 0)
    """
    ...
    for seq_idx, sequence in enumerate(sequences):
        ...
        # Process each observation in the sequence (skip BOS token at position 0 and final BOS token)
        ...
        # Add the final belief state (same as last computed state) to match activation dimensions
        sequence_beliefs.append(current_belief.copy())
...
def create_position_colored_plot(intermediate_data: np.ndarray, final_data: np.ndarray, post_layernorm_data: np.ndarray, sequences: List[List[int]]):
    """
    Create a plot colored by sequence position instead of belief states.
    """
    ...
    colors = cm.viridis(np.linspace(0, 1, seq_length_minus_1))
    
    # Panel 1: Intermediate activations
    scatter = axes[0].scatter(intermediate_pca[:, 0], intermediate_pca[:, 1], 
    ...
    scatter = axes[1].scatter(final_pca[:, 0], final_pca[:, 1], 
                             c=position_labels, cmap='viridis', alpha=0.6, s=8,
                             edgecolors='black', linewidth=0.05)
    ...
    scatter = axes[2].scatter(post_layernorm_pca[:, 0], post_layernorm_pca[:, 1], 
                             c=position_labels, cmap='viridis', alpha=0.6, s=8,
                             edgecolors='black', linewidth=0.05)
    ...
    cbar = plt.colorbar(scatter, ax=axes, orientation='horizontal', pad=0.15, shrink=0.6, aspect=30)
    cbar.set_label('Sequence Position (1=after BOS, 11=final token)', fontsize=12)
    cbar.set_ticks(range(1, seq_length_minus_1 + 1))
    
    # Save the plot
    output_path = 'play/plots/pca_position_colored_all_tokens.png'
...
def perform_pca_and_plot(intermediate_data: np.ndarray, final_data: np.ndarray, post_layernorm_data: np.ndarray, sequences: List[List[int]]):
    ...
    # Save the plot
    output_path = 'play/plots/pca_vs_theoretical_nine_panel_all_tokens.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def main():
    ...
    # Create additional plot colored by sequence position
    create_position_colored_plot(intermediate_processed, final_processed, post_layernorm_processed, sequences)
