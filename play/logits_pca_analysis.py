#!/usr/bin/env python3
"""
Script to perform PCA analysis on logits (post-unembed, pre-softmax) from a GPT model.
Generates all possible binary sequences, captures logits activations,
and visualizes the first principal components.
"""
...
def capture_logits_activations(model: GPT, sequences: List[List[int]], device: torch.device) -> torch.Tensor:
    """
    Capture logits activations (post-unembed, pre-softmax) for a batch of sequences.
    Returns logits from the language modeling head.
    """
    ...
    with torch.no_grad():
        # Forward pass through the complete model to get logits
...
def process_activations(activations: torch.Tensor, remove_bos: bool = True) -> np.ndarray:
    ...
    Input shape: (num_sequences, seq_len, vocab_size)
    Output shape: (num_sequences * seq_len_processed, vocab_size)
    """
    if remove_bos:
        # Remove BOS token (position 0) and final position (position -1)
        activations_processed = activations[:, 1:-1, :]  # Shape: (num_sequences, seq_len-2, vocab_size)
    else:
        activations_processed = activations
    
    # Flatten over sequence dimension
    num_sequences, seq_len_processed, vocab_size = activations_processed.shape
    flattened = activations_processed.reshape(num_sequences * seq_len_processed, vocab_size)
...
def perform_pca_and_plot(activations: np.ndarray, sequences: List[List[int]], vocab_size: int):
    """
    Perform PCA on logits activation data and create visualization.
    """
    print("Performing PCA analysis...")
    
    # For vocab_size=3, we can have at most 3 components
    n_components = min(3, activations.shape[1])
    pca = PCA(n_components=n_components)
    pca_result = pca.fit_transform(activations)
    
    print(f"PCA explained variance ratio: PC1={pca.explained_variance_ratio_[0]:.4f}, PC2={pca.explained_variance_ratio_[1]:.4f}", end="")
    if n_components >= 3:
    ...
    ax.set_title(f'PCA: Logits (Post-Unembed, Pre-Softmax) (Colored by Proportion of 0\'s)\nPC1: {pca.explained_variance_ratio_[0]:.3f}, PC2: {pca.explained_variance_ratio_[1]:.3f}')
    ...
    # Save the 2D plot
    output_path_2d = 'play/plots/logits_pca_analysis.png'
    plt.savefig(output_path_2d, dpi=300, bbox_inches='tight')
    print(f"2D plot saved to {output_path_2d}")
    
    # Create interactive 3D plot with Plotly (if we have 3 dimensions)
    if n_components >= 3:
        ...
        fig_3d.update_layout(
            title=f'Interactive 3D PCA: Logits (Post-Unembed, Pre-Softmax) (Colored by Proportion of 0\'s)<br>PC1: {pca.explained_variance_ratio_[0]:.3f}, PC2: {pca.explained_variance_ratio_[1]:.3f}, PC3: {pca.explained_variance_ratio_[2]:.3f}',
            scene=dict(
        ...
        output_path_3d = 'play/plots/logits_pca_analysis_3d.html'
        fig_3d.write_html(output_path_3d)
        print(f"Interactive 3D plot saved to {output_path_3d}")
    else:
        print("Skipping 3D plot - not enough dimensions for 3D visualization")
    
    # Print summary statistics
    print(f"\n=== Logits PCA Results Summary ===")
...
def main():
    print("=== Logits (Post-Unembed, Pre-Softmax) PCA Analysis ===\n")
    ...
    for i in range(num_batches):
        ...
        # Capture logits activations for this batch
        logits_batch = capture_logits_activations(model, batch_sequences, device)
        all_logits.append(logits_batch)
    
    # Concatenate all batches
    print("Concatenating all batches...")
    logits_activations = torch.cat(all_logits, dim=0)
    print(f"Logits activations shape: {logits_activations.shape}")
    
    # Process activations (remove BOS token and final position, then flatten)
    print("Processing activations...")
    processed_activations = process_activations(logits_activations, remove_bos=True)
    print(f"Processed activations shape: {processed_activations.shape}")
    
    # Perform PCA and create plots
    perform_pca_and_plot(processed_activations, sequences, vocab_size)
